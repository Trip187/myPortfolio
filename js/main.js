...
(function (html) {
  ...
  const ssPreloader = function () {
    ...
    function finishPreloader() {
      ...
      // hide the preloader element after animation starts
      setTimeout(function () {
    ...
    // If the document is already loaded, finish immediately
    if (document.readyState === "complete") {
      finishPreloader();
      return;
    }

    // Run as soon as DOM is ready to improve perceived performance
    window.addEventListener("DOMContentLoaded", function () {
      // slight delay to let styles settle
      setTimeout(finishPreloader, 80);
    });

    // Also finish on full load if that happens later
    window.addEventListener("load", finishPreloader);

    // Safety fallback in case load never fires (e.g., blocked resources)
